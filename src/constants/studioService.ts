import type { StudioServiceCode, StudioServiceTag } from '@/types/studio'

// serviceCode enum 4종 전부에 대한 표시 라벨.
// 필터 목록(STUDIO_SERVICE_FILTER_CODES)은 WIFI를 노출하지 않지만 응답에는 내려오므로,
...
// 검색 필터에 노출하는 서비스 코드와 칩 표시 순서. 라벨은 위 표시용 맵을 그대로 쓴다.
export const STUDIO_SERVICE_FILTER_CODES = [
  'HAIR_MAKEUP',
  'COSTUME',
  'PARKING',
] as const satisfies readonly StudioServiceTag[]

export const isStudioServiceTag = (value: string): value is StudioServiceTag =>
  (STUDIO_SERVICE_FILTER_CODES as readonly string[]).includes(value)
