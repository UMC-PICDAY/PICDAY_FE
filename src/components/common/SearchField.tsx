import type { ChangeEventHandler, FormEvent, FormEventHandler, MouseEventHandler } from 'react'
  ...
  if (props.variant === 'result') {
    ...
    return (
      ...
      >
        ...
        <span className={selected ? 'text-[var(--color-brand-100)]' : 'text-[var(--color-gray-40)]'}>
          {resultMeta}
        </span>
      </button>
    )
  ...
  return (
    <form
      className={`group flex h-12 w-full items-center gap-2.5 rounded-lg border bg-[var(--color-white)]/75 px-4 shadow-[0_15px_40px_rgba(206,206,206,0.08)] backdrop-blur-[10px] focus-within:border-[var(--color-gray-60)] ${
        isActive ? 'border-[var(--color-gray-60)]' : 'border-[var(--color-gray-10)]/60'
