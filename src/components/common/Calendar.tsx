...
const getMonthByOffset = ({ year, month }: CalendarMonth, offset: number): CalendarMonth => {
  ...
  return (
    ...
    >
      ...
      {months.map(({ year, month }) => {
        ...
        const leadingEmptyCells = Array.from({ length: firstWeekday }, (_, index) => index)
          ...
          <Fragment key={`${year}-${month}`}>
            ...
            <div className="grid w-full grid-cols-7 gap-x-6 gap-y-3">
              {leadingEmptyCells.map((cell) => (
                <span key={`empty-${year}-${month}-${cell}`} className="size-7" aria-hidden />
              ))}
              {days.map((day) => {
                ...
                return (
                  <DayCell
                    key={day}
                    day={day}
                    selected={selectedDate !== undefined && isSameDate(selectedDate, date)}
                    disabled={disabled}
